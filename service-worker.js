...
async function getAppVersion() {
    try {
        const resp = await fetch(resolveUrl(APP_VERSION_URL), { cache: 'no-store' });
        ...
        CACHE_NAME_PROMISE.then((cacheName) => caches.open(cacheName))
            .then((cache) => {
                ...
                return Promise.allSettled(
                    CACHE_ASSETS.map((url) => cache.add(resolveUrl(url)))
                );
            })
            .then(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (req.mode === 'navigate') {
                    const fallback = await caches.match(resolveUrl(OFFLINE_FALLBACK_URL));
                    if (fallback) return fallback;
                }
